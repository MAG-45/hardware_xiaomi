...
#else
    int error = mDevice->revokeChallenge(mDevice, challenge);
    if (error) {
        ALOGI("Failed to revoke challenge=%ld, error=%d", challenge, error);
    }
#endif
...
ndk::ScopedAStatus Session::removeEnrollments(const std::vector<int32_t>& enrollmentIds) {
    ...
    if (error != 0) {
        ...
        mCb->onError(Error::UNABLE_TO_PROCESS, error /* vendorError */);
...
ndk::ScopedAStatus Session::resetLockout(const HardwareAuthToken& hat) {
    ...
    if (resetResult != 0) {
        ...
        mCb->onError(Error::UNABLE_TO_PROCESS, resetResult /* vendorError */);
        return ndk::ScopedAStatus::fromServiceSpecificError(resetResult);
    }
#endif
...
void Session::notify(const fingerprint_msg_t* msg) {
    ...
    switch (msg->type) {
        ...
        case FINGERPRINT_CHALLENGE_GENERATED: {
            ALOGD("onChallengeGenerated(%lu)", msg->data.challenge_generated.challenge);
            mCb->onChallengeGenerated(msg->data.challenge_generated.challenge);
        } break;
        case FINGERPRINT_CHALLENGE_REVOKED: {
            ALOGD("onChallengeRevoked(%lu)", msg->data.challenge_revoked.challenge);
            mCb->onChallengeRevoked(msg->data.challenge_revoked.challenge);
        } break;
        case FINGERPRINT_AUTHENTICATOR_ID_RETRIEVED: {
            ALOGD("onAuthenticatorIdRetrieved(%lu)",
                  msg->data.authenticator_id_retrieved.authenticator_id);
            mCb->onAuthenticatorIdRetrieved(msg->data.authenticator_id_retrieved.authenticator_id);
        } break;
        case FINGERPRINT_AUTHENTICATOR_ID_INVALIDATED: {
            ALOGD("onAuthenticatorIdInvalidated(%lu)",
                  msg->data.authenticator_id_invalidated.authenticator_id);
            mCb->onAuthenticatorIdInvalidated(
                    msg->data.authenticator_id_invalidated.authenticator_id);
        } break;
